...
const Section: React.FC<{title: string, icon: React.ReactNode, children: React.ReactNode, defaultOpen?: boolean}> = ({ title, icon, children, defaultOpen = true }) => {
  ...
  return (
    <div className="mb-4 bg-white dark:bg-[#181c2a] rounded-xl shadow-xl border border-gray-100 dark:border-[#232946] p-6 transition-all duration-300 hover:shadow-2xl hover:border-blue-400 group">
      <button
        ...
        aria-expanded={open}
      >
        <span className="flex items-center gap-2 text-blue-900 dark:text-blue-400 font-bold text-xl">{icon}{title}</span>
        <span className="block h-1 w-8 animated-accent-bar bg-gradient-to-r from-blue-600 to-purple-600 rounded-full mt-1 group-hover:w-16 group-hover:accent-bar-animate transition-all duration-300"></span>
...
// Tooltip component for accessibility and style
const Tooltip: React.FC<{ text: string; children: React.ReactNode }> = ({ text, children }) => (
  <span className="relative group inline-block">
    {children}
    <span className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-max px-2 py-1 rounded bg-gray-900 text-white text-xs opacity-0 group-hover:opacity-100 group-focus:opacity-100 pointer-events-none transition-opacity z-50 shadow-lg whitespace-nowrap">
      {text}
    </span>
  </span>
);
...
const ProfileView: React.FC = () => {
  ...
  // Fix: Call useCountUp hooks at the top level
  const connectionsCount = useCountUp(mockStats.connections);
  const mutualConnectionsCount = useCountUp(mockStats.mutualConnections);
  const postsCount = useCountUp(mockStats.posts);
  const endorsementsCount = useCountUp(mockStats.endorsements);
      ...
      <div className="w-full max-w-4xl rounded-2xl shadow-xl overflow-visible card-hover bg-main relative">
        ...
        <div className="flex justify-center absolute left-0 right-0" style={{ top: '120px' }}>
          <div className="group">
            ...
            <div className="profile-gradient-border w-44 h-44 rounded-full flex items-center justify-center transition-transform duration-300 group-hover:scale-105">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mt-28 px-8 pb-2">
          ...
          <div className="flex gap-8 mt-4 md:mt-0">
            ...
            <div className="flex gap-8 bg-white rounded-xl shadow px-6 py-3 items-center">
              <div className="flex flex-col items-center" title="Connections" aria-label="Connections">
                <span className="font-bold text-lg text-blue-700 flex items-center gap-1">
                  <svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87M16 3.13a4 4 0 010 7.75M8 3.13a4 4 0 000 7.75" /></svg>
                  {connectionsCount}
                </span>
                <span className="text-xs text-gray-500">Connections</span>
              </div>
              <div className="flex flex-col items-center" title="Mutual Connections" aria-label="Mutual Connections">
                <span className="font-bold text-lg text-purple-700 flex items-center gap-1">
                  <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" /></svg>
                  {mutualConnectionsCount}
                </span>
                <span className="text-xs text-gray-500">Mutual</span>
              </div>
              <div className="flex flex-col items-center" title="Posts" aria-label="Posts">
                <span className="font-bold text-lg text-blue-700 flex items-center gap-1">
                  <svg className="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21H5a2 2 0 01-2-2V7a2 2 0 012-2h4l2-2h6a2 2 0 012 2v12a2 2 0 01-2 2z" /></svg>
                  {postsCount}
                </span>
                <span className="text-xs text-gray-500">Posts</span>
              </div>
              <div className="flex flex-col items-center" title="Endorsements" aria-label="Endorsements">
                <span className="font-bold text-lg text-purple-700 flex items-center gap-1">
        ...
        <div className="mt-8 px-4 md:px-8 pb-10">
          <motion.div
            ...
            whileHover={{ scale: 1.025, boxShadow: '0 8px 32px 0 rgba(99,102,241,0.10)' }}
            className="mb-4"
          >
